export default async function handler(req, res) {
  ...
  const extras = [
    length ? `Make the response ${length}.` : '',
  ...
  if (mode === 'improve' && original) {
    ...
    prompt = `Write ${count} different ${category}${count > 1 ? 's' : ''} about "${topic}"${tone ? ` in a ${tone} tone` : ''}. ${extras}`;
  }
  ...
  try {
    ...
    // Return string directly so frontend treats it as a single prompt block
